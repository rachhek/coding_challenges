...
class StatsCollector {
	...
	getMedian() /*number*/ {
    let medianCalculator = new MedianCalculator(this.responseArr,1,this.responseTimeOut)
...
class MedianCalculator{
  ...
  getMedian(binsArr,cumulativeArr){
    ...
    }else{   //if the length of the responses is even
  ...
  calculateFrequencies(bins,values){
    ...
    let freqArr = bins.map(x=>{return 0})
...
datasets = [dataset1.sort(),
						dataset2.sort(function(a, b){return b-a}),
            dataset3,dataset4]
...
describe('StatsCollector', function(){
    ...
    datasets.forEach(function(dataset,idx){
      it('it should calculate the median for Dataset: '+ (idx+1).toString(), function(){
  ...
  datasets.forEach(function(dataset,idx){
    it('it should calculate the average for Dataset: '+ (idx+1).toString(), function(){
